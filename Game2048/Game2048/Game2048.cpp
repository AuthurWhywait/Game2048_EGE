...
void memory_score(int* scores, int index) {
	                                                                                            //REDO_TIMES is the size of the scores
	if (index < REDO_TIMES) scores[index++] = score;
	else {
		for (int i = 0; i < REDO_TIMES - 1; i++)
			scores[i] = scores[i + 1];
		scores[REDO_TIMES - 1] = score;
	}
}
...
int main() {
	...
	while (!isquit) {                                                                           //除非点击ESC，否则可以反复开局
		...
		int*** nums_assist = (int***)calloc(nums_assist_size, sizeof(int**));                   //状态矩阵数组，记录之前的矩阵状态，在撤销时候使用
		for (int i = 0; i < nums_assist_size; i++)
			nums_assist[i] = init_nums();

		int* scores = (int*)calloc(REDO_TIMES, sizeof(int));                                    //分数数组，记录之前的分数，在撤销时使用
		int scoreIndex = 0;                                                                     //分数数组的索引
		...
		while (isOK) {
			...
			case REDO:
				...
				if (scoreIndex) score = scores[(--scoreIndex)-1];                               //回退为上一个被记录的分数
				break;
			case LEFT:
				left(nums);                                                                     //两次调用方向函数是为了使消除完全彻底，下同
				left(nums);
				memory_score(scores, scoreIndex++);
				break;
			case RIGHT:
				right(nums);
				right(nums);
